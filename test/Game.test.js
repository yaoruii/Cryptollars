...
describe("Game Contract Test", function () {
  it("1. Function fails if the monster does not exist.", async function () {
    ...
    await expect(gameTest.connect(player1).attack_monster(no_monster)).to.be
      .reverted;
  });
  ...
  it("2. Function succeeds if player kills monster, modifies player’s equipment number will increase by one, money will increase and health will decrease.", async function () {
    ...
    const health_before = await gameTest
      .connect(admin)
      .get_current_health(player1.address);
    const storage_before = await gameTest
      .connect(admin)
      .get_storage_len(player1.address);
    ...
    //check after
    const health_after = await gameTest
      .connect(admin)
      .get_current_health(player1.address);
    let storage_after = await gameTest
      .connect(admin)
      .get_storage_len(player1.address);
  ...
  it("3. Function succeeds if monster kills player, play’s health will equal to max health, and player’s equipment number will decrease by one. If player has one equipment, no equipment number decrease.", async function () {
    ...
    const money_after = await gameTest
      ...
      .balanceOf(player1.address, 1);
    if (storage_before > 1) {
      expect(Number(storage_before - 1)).to.equal(Number(storage_after));
    } else {
      expect(Number(storage_before)).to.equal(Number(storage_after));
    }
    expect(Number(money_after)).to.equal(Number(money_before));
    const full_health = await gameTest
      .connect(admin)
      .get_max_health(player1.address);
  ...
  it("1. Function fails if the opponent does not exist.", async function () {
    ...
    const gameTest = await game.connect(admin).deploy();
    await gameTest.connect(player1).initialize("Jack");
    await expect(gameTest.connect(player1).invite_duel(player2.address)).to.be
      .reverted;
  });
  ...
  it("2. Function fails if already sent out a pending invite.", async function () {
    ...
    await expect(gameTest.connect(admin).invite_duel(player1.address)).to.be
      .reverted;
  });
  ...
  it("3. Function succeeds, changes invite status.", async function () {
    ...
    let is_pen = await gameTest
      .connect(player1)
      .get_is_pending(player1.address);
  ...
  it("1. Function fails if player does not receive invitation.", async function () {
    ...
    await gameTest.connect(player1).initialize("Jack");
    await gameTest.connect(player2).initialize("Rose");
    await expect(gameTest.connect(player1).accept_duel(player2.address)).to.be
      .reverted;
  });
  ...
  it("2. Function succeeds, inviter wins, inviter’s equipment number will increase by one, and invitee’s equipment number will decrease by one. Invitee will reborn with max health. Duel status will become proceed.", async function () {
    ...
    let inviter_before = await gameTest
      .connect(admin)
      .get_storage_len(player1.address);
    let invitee_before = await gameTest
      .connect(admin)
      .get_storage_len(admin.address);
    //accept
    await gameTest.connect(admin).accept_duel(player1.address);

    let inviter_after = await gameTest
      .connect(admin)
      .get_storage_len(player1.address);
    let invitee_after = await gameTest
      .connect(admin)
      .get_storage_len(admin.address);
    const health_after = await gameTest
      .connect(admin)
      .get_current_health(admin.address);
    const full_health = await gameTest
      .connect(admin)
      .get_max_health(admin.address);
    expect(Number(health_after)).to.equal(Number(full_health));
    if (invitee_before > 1) {
  ...
  it("3. Function succeeds, invitee wins, invitee’s equipment number will increase by one, and inviter’s equipment number will decrease by one. Inviter will reborn with max health. Duel status will become proceed.", async function () {
    ...
    let invitee_before = await gameTest
      .connect(admin)
      .get_storage_len(player2.address);
    //accept
  ...
  it("4. Function succeeds, after duel, the player’s status becomes open.", async function () {
    ...
    let is_pen = await gameTest
      .connect(player2)
      .get_is_pending(player2.address);
  ...
  it("1. Function fails if player does not receive invitation. ", async function () {
    ...
    const gameTest = await game.connect(admin).deploy();
    await gameTest.connect(player1).initialize("Jack");
    await expect(gameTest.connect(player1).reject_duel(player2)).to.be.reverted;
  });
  ...
  it("3. Function succeeds, player’s attack strength will increase by equipment strength.", async function () {
    ...
    const attack_before = await gameTest
      .connect(admin)
      .get_attack(player1.address);
    //equip new sowrd
    await gameTest.connect(player1).equip(2);
    const attack_after = await gameTest
      .connect(admin)
      .get_attack(player1.address);
  ...
  it("4. Function succeeds, player’s equipment slot will be occupied.", async function () {
    ...
    await gameTest.connect(player1).equip(2);
    const equip_id = await gameTest
      .connect(admin)
      .get_equip_id(player1.address);
  ...
  it("4. get all players successfully.", async function () {
    ...
    //unequip new sword
    await gameTest.connect(player1).unequip();
    const num_of_players = await gameTest.connect(admin).get_all_players();

    const addr_of_player1 = await gameTest
      .connect(player1)
      .get_address(player1.address);
